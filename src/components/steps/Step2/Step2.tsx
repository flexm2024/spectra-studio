...
import { createParticleOverlayState, tickParticleOverlay } from '../../../lib/visualizer/particleOverlay'
import type { ParticleOverlayState } from '../../../lib/visualizer/particleOverlay'
...
export default function Step2({ tracks, theme, setTheme, effects, setEffects, visualizer, setVisualizer, typography, setTypography, onBack, onNext, playingId, isPlaying, onPlay, onPause, onSkipNext, onSkipPrev, background, logo, logoPosition, setLogoPosition, logoSize, setLogoSize, currentTime, onSeek, analyserRef, particleOverlay, setParticleOverlay }: Step2Props) {
  ...
  const particleOverlayStateRef = useRef<ParticleOverlayState | null>(null)
  const particleOverlayRef = useRef(particleOverlay)
  particleOverlayRef.current = particleOverlay
  ...
  useEffect(() => {
    ...
    particleOverlayStateRef.current = createParticleOverlayState(particleOverlay)

    let rafId: number
    function tick() {
      const W = canvas!.width, H = canvas!.height
      ctx!.clearRect(0, 0, W, H)
      tickParticleOverlay(ctx!, particleOverlayStateRef.current!, particleOverlayRef.current, W, H)
      rafId = requestAnimationFrame(tick)
    }
    rafId = requestAnimationFrame(tick)
    return () => { cancelAnimationFrame(rafId); ctx?.clearRect(0, 0, canvas!.width, canvas!.height) }
